document.addEventListener('DOMContentLoaded', async () => {
    ...
    const eraIcons = {
        ...
        try {
            ...
            if (!response.ok) throw new Error(`failed to load the data ${url}`);
    ...
    const renderEpisodes = (episodeList) => {
        ...
        const headerRow = episodeContainer.querySelector('.episode-row-header');
        if (!headerRow) {
            const header = document.createElement('div');
            header.className = 'episode-row episode-row-header';
            header.innerHTML = `
                <div class="episode-cell">Rank</div>
                <div class="episode-cell">Title</div>
                <div class="episode-cell">Series</div>
                <div class="episode-cell">Era</div>
                <div class="episode-cell">Year</div>
                <div class="episode-cell">Director</div>
                <div class="episode-cell">Writer</div>
                <div class="episode-cell">Doctor</div>
                <div class="episode-cell">Companion</div>
                <div class="episode-cell">Cast Count</div>
                <div class="episode-cell">Plot</div>
            `;
            episodeContainer.appendChild(header);
        }
        const episodeRows = episodeContainer.querySelectorAll('.episode-row:not(.episode-row-header)');
        episodeRows.forEach(row => row.remove());
